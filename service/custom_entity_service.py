...
1. summary — 案件简要描述（固定格式总结，每篇文书仅生成 1 个）
   ...
   注意：仅 description 字段，无 attributes
...
5. PreventionMeasure — 防范建议（基于案情推理生成，不局限于原文）
   例如：核实平台资质、警惕高收益承诺、不向个人账户转账、保留聊天记录
   说明：判决书通常不写防范措施，需要根据案情推理出可行的预防建议
   建议属性：防范对象、适用人群、可操作性
...
1. involves（summary → FraudScenario）
   案件摘要涉及某类诈骗场景
   例：(summary) involves 虚假网络投资理财类诈骗

2. describes（summary → FraudFeature）
   案件摘要描述了某个骗局特征
   例：(summary) describes 虚构高收益投资平台

3. mentions（summary → FraudMethod）
   案件摘要提及某种诈骗手法
   例：(summary) mentions 伪造交易截图
...
class CustomEntityService:
    ...
    def insert_with_custom_schema(
        ...
        # 1) 先从图谱获取所有已有实体名做精确去重
        # 2) 对未命中的实体做 embedding 相似度消歧
        existing_names = set()
